...
dig.data = (function() {
   ...
   *
   * This constructor takes an optional array of values used to initialize
   * the heap.
   */
  function binaryHeap(init) {
    ...
    function _heapify(i) {
      var l = 2 * i,
          r = l + 1,
          largest;
      if (l < _arr.length && _arr[l] < _arr[i]) {
        largest = l; 
      } else {
        largest = i;
      }

      if (r < _arr.length && _arr[r] < _arr[largest]) {
        largest = r;
      }
     ...
     */
    function add(key) {
      _arr.push(Number.POSITIVE_INFINITY);
      _decreaseKey(_arr.length - 1, key);
    }
